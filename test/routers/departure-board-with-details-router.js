...
const DepartureBoardRouter = require('../../routers/departure-board-with-details-router')

describe('DepartureBoardWithDetailsRouter', () => {
  let router
  let rail
  let parentLog
  let log
  ...
  describe('constructor', () => {
    it('should setup availabe URIs and map to internal methods to handle response', () => {
      ...
      expect(parentLog.child).to.be.calledWithExactly({
        module: 'departure-board-with-details-router'
      })
      expect(departureBoardRouter._log).to.deep.equal(log)
      expect(departureBoardRouter._method.name).to.equal('bound getDepartureBoardWithDetails')
    })
  })
})
